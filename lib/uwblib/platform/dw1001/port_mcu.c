...
volatile uint32_t time32_incr = 0;
...
{
  // nrf_drv_systick_state_t state;
  // nrf_drv_systick_get(&state);
  // return state.time;
	// return systickGetTick();
	// return HAL_GetTick();
...
unsigned long long portGetTickCntMicro(void)
{
	...
	{
		microsec = major_incr1*1000 + 1000 - (uint64_t)(minor_incr1/64);
	}
	else
	{
		microsec = major_incr2*1000 + 1000 - (uint64_t)(minor_incr2/64);
	}
